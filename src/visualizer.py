...
class DataVisualizer:
        ...
        try:
            ...
            spending_totals = {
                col.replace(
                    ...
                    '').replace(
                    '_',
                    ' ').title(): self.data[col].sum() for col in spending_cols}

            axes[0, 0].pie(spending_totals.values(),
                           labels=spending_totals.keys(), autopct='%1.1f%%')
            ...
            avg_spending = {
                col.replace(
                    ...
                    '').replace(
                    '_',
                    ' ').title(): self.data[col].mean() for col in spending_cols}

            categories = list(avg_spending.keys())
            values = list(avg_spending.values())

            bars = axes[0, 1].bar(
                categories, values, color=sns.color_palette("husl", len(categories)))
            ...
            for bar, value in zip(bars, values):
                axes[0, 1].text(bar.get_x() +
                                bar.get_width() /
                                2, bar.get_height() +
                                value *
                                0.01, f'${value:.0f}', ha='center', va='bottom')
            ...
            if 'age' in self.data.columns:
                ...
                p = np.poly1d(z)
                axes[1, 0].plot(self.data['age'], p(
                    self.data['age']), "r--", alpha=0.8)

            # Chart 4: Spending distribution (histogram)
            total_spending = self.data[spending_cols].sum(axis=1)
            axes[1, 1].hist(total_spending, bins=10,
    ...
    def create_savings_charts(self, save_path=None):
        ...
        try:
            ...
            axes[0, 0].hist(self.data['monthly_savings'], bins=12,
            ...
            if 'annual_income' in self.data.columns:
                axes[0,
                     1].scatter(self.data['annual_income'],
                ...
                z = np.polyfit(
                    ...
                    1)
                p = np.poly1d(z)
                axes[0, 1].plot(self.data['annual_income'], p(
                    self.data['annual_income']), "r--", alpha=0.8)
            ...
            if 'annual_income' in self.data.columns and 'age' in self.data.columns:
                ...
                age_groups = pd.cut(
                    self.data['age'], bins=[
                        0, 30, 40, 50, 100], labels=[
                        '<30', '30-40', '40-50', '50+'])
                avg_savings_rate = savings_rate.groupby(age_groups).mean()

                bars = axes[1,
                            0].bar(range(len(avg_savings_rate)),
                ...
                for bar, value in zip(bars, avg_savings_rate.values):
                    axes[1,
                         0].text(bar.get_x() + bar.get_width() / 2,
    ...
    def create_investment_charts(self, save_path=None):
        ...
        try:
            ...
            if 'primary_investment' in self.data.columns:
                ...
                axes[0, 0].pie(investment_counts.values,
                               labels=investment_counts.index, autopct='%1.1f%%')
            ...
            if 'primary_investment' in self.data.columns and 'age' in self.data.columns:
                ...
                axes[1, 0].legend(title='Investment Type',
    ...
    def create_financial_literacy_charts(self, save_path=None):
        ...
        try:
            ...
            axes[0, 0].hist(self.data['financial_literacy_score'],
            ...
            if 'annual_income' in self.data.columns:
                axes[0,
                     1].scatter(self.data['financial_literacy_score'],
                                self.data['annual_income'],
                                alpha=0.6,
                                color='purple')
                ...
                z = np.polyfit(
                    ...
                    1)
                p = np.poly1d(z)
                axes[0, 1].plot(self.data['financial_literacy_score'], p(
                    self.data['financial_literacy_score']), "r--", alpha=0.8)
            ...
            if 'age' in self.data.columns:
                ...
                literacy_by_age = self.data.groupby(
                    age_groups)['financial_literacy_score'].mean()

                bars = axes[1,
                            0].bar(range(len(literacy_by_age)),
    ...
    def create_comprehensive_dashboard(self, save_path=None):
        ...
        try:
            ...
            if 'uses_mobile_banking' in self.data.columns and 'owns_crypto' in self.data.columns:
                ...
                ax5.bar(['Mobile Banking', 'Crypto'], [
                        mobile, crypto], color=['blue', 'orange'])
            ...
            table = ax9.table(
                cellText=summary_data, colLabels=[
                    'Metric', 'Value'], cellLoc='center', loc='center', bbox=[
                    0.2, 0.2, 0.6, 0.6])
