...
class FinanceAnalyzer:
    ...
    def get_spending_analysis(self):
        ...
        analysis["Spending Overview"] = {
            "Average Total Spending": format_currency(
                self.data['total_spending'].mean()), "Median Total Spending": format_currency(
                self.data['total_spending'].median()), "Spending Range": f"{
                format_currency(
                    self.data['total_spending'].min())} - {
                        format_currency(
                            self.data['total_spending'].max())}"}

        # Category breakdown
        for col in spending_cols:
            category_name = col.replace(
                'monthly_spending_', '').replace(
                '_', ' ').title()
    ...
    def get_savings_analysis(self):
        ...
        analysis["Savings Overview"] = {
            "Average Monthly Savings": format_currency(
                self.data['monthly_savings'].mean()), "Median Monthly Savings": format_currency(
                self.data['monthly_savings'].median()), "Savings Range": f"{
                format_currency(
                    self.data['monthly_savings'].min())} - {
                        format_currency(
                            self.data['monthly_savings'].max())}"}

        # Savings rate analysis (if income data available)
        if 'annual_income' in self.data.columns:
            monthly_income = self.data['annual_income'] / 12
            self.data['savings_rate'] = self.data['monthly_savings'] / \
                monthly_income
            ...
            # Generate insight
            high_savers_pct = len(
                self.data[self.data['savings_rate'] > 0.2]) / len(self.data)
            analysis["Insights"].append(
    ...
    def get_fintech_adoption_analysis(self):
        ...
        if 'uses_mobile_banking' in self.data.columns and 'owns_crypto' in self.data.columns:
            ...
            analysis["Digital Adoption Patterns"] = {
                "Tech Enthusiasts (Both)": f"{
                    len(tech_enthusiasts)} respondents ({
                    format_percentage(
                        len(tech_enthusiasts) / len(
                            self.data))})"}
    ...
    def get_financial_literacy_analysis(self):
        ...
        analysis["Literacy Overview"] = {
            "Average Score": f"{
                self.data['financial_literacy_score'].mean():.1f}/10",
            "Median Score": f"{
                self.data['financial_literacy_score'].median():.1f}/10",
            "Score Range": f"{
                self.data['financial_literacy_score'].min():.0f} - {
                    self.data['financial_literacy_score'].max():.0f}"}
    ...
    def get_comprehensive_report(self):
        ...
        if not self.data.empty:
            report["Executive Summary"] = {
                "Total Respondents": len(
                    self.data),
                "Average Age": f"{
                    self.data['age'].mean():.1f} years" if 'age' in self.data.columns else "N/A",
                "Average Income": format_currency(
                    self.data['annual_income'].mean()) if 'annual_income' in self.data.columns else "N/A",
                "Average Monthly Savings": format_currency(
                    self.data['monthly_savings'].mean()) if 'monthly_savings' in self.data.columns else "N/A"}
